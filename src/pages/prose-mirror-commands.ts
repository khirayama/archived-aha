...
export function deleteSelection(state, dispatch) {
  ...
  if (state.selection.empty) {
    console.log('deleteSelection: selection empty');
    return false;
  }
  if (dispatch) {
    console.log('deleteSelection: dispatch');
...
export function joinBackward(state, dispatch, view) {
  ...
  if (!$cut) {
    ...
    if (target == null) {
      console.log('joinBackward: no target');
      return false;
    }
    if (dispatch) {
      console.log('joinBackward: dispatch scrollIntoView');
  ...
  if ($cursor.parent.content.size == 0 && (textblockAt(before, 'end') || NodeSelection.isSelectable(before))) {
    if (dispatch) {
      console.log('joinBackward: dispatch setSelection and scrollIntoView');
  ...
  if (before.isAtom && $cut.depth == $cursor.depth - 1) {
    if (dispatch) {
      console.log('joinBackward: dispatch delete and scrollIntoView');
      dispatch(state.tr.delete($cut.pos - before.nodeSize, $cut.pos).scrollIntoView());
    }
    return true;
  }

  console.log('joinBackward: return false');
  return false;
}
...
export function selectNodeBackward(state, dispatch, view) {
  console.log('selectNodeBackward: call');
  let { $head, empty } = state.selection,
    $cut = $head;
  if (!empty) return false;

  if ($head.parent.isTextblock) {
    console.log('joinBackward: isTextblock');
  ...
  if (dispatch) {
    console.log('joinBackward: dispatch setSelection');
    dispatch(state.tr.setSelection(NodeSelection.create(state.doc, $cut.pos - node.nodeSize)).scrollIntoView());
  }
  console.log('selectNodeBackward: return true');
  return true;
}
...
export function selectNodeForward(state, dispatch, view) {
  ...
  console.log('selectNodeForward: return true');
  return true;
}
